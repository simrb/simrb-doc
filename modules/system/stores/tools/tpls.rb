module Simrb
	module Stool
		...
		def system_tpl_helper modulename
			...
			tpl << "\tend\n\n"
			tpl << "end"
			{"modules/#{modulename}/#{Simrb::Sdir[:logic]}/helpers.rb" => tpl}
		end
		...
		def system_tpl_admin modulename
			...
			{"modules/#{modulename}/#{Simrb::Sdir[:logic]}/admin.rb" => tpl}
		end

		# /views/name_layout.slim
		def system_tpl_layout modulename
			@et = { :name => modulename }
			tpl	= system_get_erb("modules/#{Scfg[:main_module]}/#{Simrb::Sdir[:docs]}/layout.erb")
			{"modules/#{modulename}/#{Simrb::Sdir[:view]}/#{modulename}_layout.slim" => tpl}
		end

		# /stores/assets/name.css
		def system_tpl_layout_css modulename
			tpl = ""
			path = "modules/system/#{Simrb::Sdoc[:layout_css]}"
			if File.exist? path
				tpl << File.read(path)
			end
			{"modules/#{modulename}/#{Simrb::Sdir[:assets]}/#{modulename}.css" => tpl}
		end

		def system_tpl_common_css modulename
			tpl = ""
			path = "modules/system/#{Simrb::Sdoc[:common_css]}"
			if File.exist? path
				tpl << File.read(path)
			end
			{"modules/#{modulename}/#{Simrb::Sdir[:assets]}/#{modulename}_common.css" => tpl}
		end

		# /stores/assets/name.js
		def system_tpl_js modulename
			tpl = ""
			{"modules/#{modulename}/#{Simrb::Sdir[:assets]}/#{modulename}.js" => tpl}
		end
		...
		def system_tpl_menu modulename
			...
			require 'yaml'
			{"modules/#{modulename}/#{Simrb::Sdir[:install]}/_menu" => tpl.to_yaml}
		end
		...
		def system_tpl_var modulename
			...
			require 'yaml'
			{"modules/#{modulename}/#{Simrb::Sdir[:install]}/_vars" => tpl.to_yaml}
		end
