module Simrb
	module Stool
		...
		def g_vars args
			...
			path		= system_add_suffix("modules/#{module_name}/#{Sbase::File[:vars]}")
			content		= system_get_erb("modules/#{Scfg[:main_module]}/#{Sbase::Dir[:docs]}/vars.erb")
		...
		def g_menu args
			...
			path		= system_add_suffix("modules/#{module_name}/#{Sbase::File[:menu]}")
			content		= system_get_erb("modules/#{Scfg[:main_module]}/#{Sbase::Dir[:docs]}/menu.erb")
			system_generate_file({path => content})
		end
		...
		def g_m2 args
			if args.count == 2
				...
				dir 	= "modules/#{modulename}/#{Sbase::Dir[:schema]}"
		...
		def g_data args, auto = true
				...
				if item.include?(":")
					...
					if arr.length > 0
 						unless arr[0].include?('=')
							...
							# normal field type
							if Sbase::Alias.keys.include? type
								data[field][:type] = Sbase::Alias[type]
			...
			data.each do | field, vals |
				Sbase::Alias.keys.each do | key |
					if data[field].include? key
						data[field][:type] 		= Sbase::Alias[key]
			...
			path 	= "modules/#{module_name}/#{Sbase::Dir[:logic]}/data.rb"
...
# 				f.write res
# 			end
			res << 'generate the above content'
		end
