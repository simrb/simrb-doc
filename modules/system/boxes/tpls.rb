module Simrb
	module Stool
		...
		def system_tpl_admin modulename
			...
			{"modules/#{modulename}/admin.rb" => tpl}
		end

		# /views/name_layout.slim
		def system_tpl_layout modulename
			@et = { :name => modulename }
			tpl	= system_get_erb("modules/#{Scfg[:main_module]}#{Spath[:tpl]}layout.erb")
		...
		def system_tpl_gitignore modulename
			...
			{"modules/#{modulename}#{Spath[:gitgnore]}" => tpl}
		end
